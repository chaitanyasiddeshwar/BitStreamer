...
| `GET /info` (chapters) | `/info` also carries `"chapters": [{"startMs":N,"name":"…"}]` read at startup via `ffprobe -show_chapters` (any container ffprobe understands) and `"thumbnails": bool` (true when ffmpeg is available). The client draws seek-bar ticks, a chapter selector, and — when thumbnails are on — a preview per chapter. |
| `GET /info` (subtitles) | `/info` also carries `"subtitles": [{"url","label","lang","mime"}]` — on-disk sidecar subtitle files next to the movie sharing its base name (`film.srt`, `film.en.srt`, `film.fr.forced.srt`; `.srt/.vtt/.ass/.ssa/.ttml/.dfxp/.sup`). Label/language are derived from the filename tag. The client adds each as a Media3 `SubtitleConfiguration` so it appears in the subtitle menu. |
| `GET /subtitle?name=<file>` (single-file) · `?path=<rel>` (folder) | Serves a sidecar subtitle file with its subtitle `Content-Type`. Single-file `name` is confined to the movie's directory (bare filename); folder `path` is confined to the served root; only known subtitle extensions are served. |
...
- **Chapters**: server reads markers via ffprobe (`chapters.go`, any container), client
